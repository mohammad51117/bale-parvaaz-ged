...
function PracticeQuestion({ question, selected, response, submitted, onSelect, onResponse, onCheck, onReset, bookmarked, onBookmark }: {
  ...
  return <article className={`practice-question ${submitted ? (isCorrect ? "is-correct" : "is-incorrect") : ""}`}>
    <div className="practice-question-head"><div className="practice-number">{String(question.number).padStart(3, "0")}</div><div><div className="eyebrow">QUESTION {question.number}</div><span className="practice-question-meta">{hasChoices ? "Select one answer" : isEssay ? "Write a response, then compare with the rubric" : "Complete the response"}</span></div><button className={`standalone-bookmark ${bookmarked ? "saved" : ""}`} onClick={onBookmark} aria-label={`Bookmark question ${question.number}`}><Bookmark size={18} fill={bookmarked ? "currentColor" : "none"} /></button></div>
...
export default function QuestionReader() {
  ...
  const questionMap = useMemo(() => new Map(allInteractiveQuestions.map((question) => [question.number, question])), [allInteractiveQuestions]);
  const activeQuestions = group.questions.map((question) => questionMap.get(question.number)).filter(Boolean) as InteractiveQuestion[];
  ...
  const reference = activeQuestions[0]?.reference || "1,001 GED Practice Questions For Dummies";
