          ...
          {showOpening ? (
            ...
            <>
              <section className="desk-hero">
                <div className="hero-copy"><div className="eyebrow"><GraduationCap size={15} /> YOUR GED PRACTICE DESK</div><h1>Pick up where your pencil left off.</h1><p>One question closer to test day. Browse the original book page by page, or jump straight into a section that needs your attention.</p><div className="hero-actions"><button className="button-primary" onClick={() => goToPage(activePage)}><BookOpen size={17} /> Continue reading</button><button className="button-text" onClick={() => setShowOpening(true)}>View the opening <ArrowRight size={16} /></button></div></div><div className="hero-art"><img src="/manus-storage/parvaaz-hero_e37cae69.jpg" alt="Open GED workbook and pencil on a warm study desk" /><div className="hero-art-caption"><span>QUESTION BANK / {formatNumber(filteredQuestionCount)} QUESTIONS</span><strong>Page-faithful practice, made navigable.</strong></div></div>
              </section>

              <section className="stats-row" aria-label="Study progress"><div className="stat-card stat-card-primary"><div className="stat-label">BOOK CONVERSION</div><div className="stat-value">{formatNumber(pages.length)} <small>pages</small></div><div className="stat-progress"><span style={{ width: `${Math.round((activePage / 683) * 100)}%` }} /></div><div className="stat-foot"><span>Current folio {activePage} of 683</span><strong>{Math.round((activePage / 683) * 100)}%</strong></div></div><div className="stat-card"><div className="stat-label">BOOKMARKED</div><div className="stat-value">{String(bookmarked.length).padStart(2, "0")} <small>pages</small></div><div className="stat-foot"><span>Saved for a second pass</span><Bookmark size={16} /></div></div><div className="stat-card"><div className="stat-label">QUESTION BANK</div><div className="stat-value">{formatNumber(filteredQuestionCount)} <small>questions</small></div><div className="stat-foot"><span>{activeWorkbookCount?.shortLabel || "All sources"}</span><strong>{formatNumber(filteredGroups.length)} sets</strong></div></div></section>
