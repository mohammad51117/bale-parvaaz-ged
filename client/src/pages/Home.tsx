...
export default function Home() {
  ...
  const groups = [...questionGroups.groups, ...supplementalEconomicsGroups] as readonly QuestionGroup[];
  ...
  const workbookCounts = useMemo(() => workbookFilterOptions.map((option) => ({ ...option, count: groups.filter((group) => matchesWorkbook(group.id, option.value)).length })), [groups]);
  ...
  const filteredPages = useMemo(() => {
    const normalized = query.toLowerCase().trim();
    if (workbookFilter === "economics") return [];
