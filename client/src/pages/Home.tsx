...
type QuestionGroup = { id: string; section: string; questionStart: number; questionEnd: number; rangeLabel: string; contextType: string; marker: string; context: string; sourcePages: readonly number[]; questions: readonly { number: number; text: string }[] };
...
export default function Home() {
  ...
  const groups = questionGroups.groups as readonly QuestionGroup[];
              ...
              <section className="reader-panel" id="question-reader">
                ...
                <div className="group-reader">
                  <div className={`group-context-panel ${activeGroup?.contextType}`}>
                    ...
                    <pre>{activeGroup?.context || "Visual source material accompanies this question set on the source folio. Keep this context in view as you work through the linked questions."}</pre>{activeGroup && ["map", "graph", "chart", "figure", "table"].includes(activeGroup.contextType) && <iframe className="source-visual-frame reader-visual" src={`/manus-storage/1001-GED-Practice-Questions-For-Dummies_530d6333.pdf#page=${activeGroup.sourcePages[0]}&view=FitH`} title={`Source visual for ${activeGroup.rangeLabel}`} />}
