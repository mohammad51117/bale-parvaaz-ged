      ...
      <aside className={`book-rail ${mobileRail ? "book-rail-open" : ""}`}>
        ...
        <nav className="rail-nav" aria-label="Study navigation">
          ...
          {bookData.subjects.map((item) => (
            <button key={item.name} className={`rail-link subject-link ${subject === item.name ? "active" : ""}`} onClick={() => { window.location.href = `/subject/${item.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "")}`; }}>
              ...
              <section className="reader-panel" id="question-reader">
                ...
                <div className="group-reader">
                  <div className={`group-context-panel ${activeGroup?.contextType}`}>
                    ...
                    {activeContextText && <pre>{activeContextText}</pre>}{activeGroup?.visualPage && visualAssets[activeGroup.visualPage] && <img className="source-visual-image reader-visual-image" src={visualAssets[activeGroup.visualPage]} alt={`Original ${activeGroup.contextType} for ${activeGroup.rangeLabel}, source folio ${activeGroup.visualPage}`} />}
