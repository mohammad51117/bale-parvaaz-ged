...
export default function SubjectPage() {
  ...
  return <div className="subject-page-shell">
    ...
    <main className="subject-page-main">
      ...
      {subject === "Social Studies" && <section className="social-lesson-catalog" aria-labelledby="social-lesson-catalog-title"><div className="social-lesson-catalog-heading"><div><div className="eyebrow"><BookOpen size={14} /> MCGRAW HILL LESSONS</div><h2 id="social-lesson-catalog-title">Study lesson by lesson.</h2></div><p>{completeSocialLessons.length} lessons across four Social Studies chapters. Each lesson opens with its complete text first, followed by its grouped source folios.</p></div><div className="social-lesson-chapter-grid">{["Civics and Government", "U.S. History", "Economics", "Geography and the World"].map((chapter) => <section className="social-lesson-chapter" key={chapter}><header><strong>{chapter}</strong><span>{completeSocialLessons.filter((lesson) => lesson.chapter === chapter).length} lessons</span></header><div>{completeSocialLessons.filter((lesson) => lesson.chapter === chapter).map((lesson, index) => <Link key={lesson.id} href={`/lesson/${lesson.id}`} className="social-lesson-row"><span className="social-lesson-number">{String(index + 1).padStart(2, "0")}</span><span className="social-lesson-title">{lesson.title}</span><span className="social-lesson-pages">{lesson.sourcePages[0]}–{lesson.sourcePages[1]}</span><ArrowUpRight size={14} /></Link>)}</div></section>)}</div></section>}
